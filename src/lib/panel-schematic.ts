...
export function buildSchematicSvg(design: PanelDesign): string {
  ...
  lines.forEach((l, i) => {
    ...
    parts.push(
      ...
      `<text x="${busX + 258}" y="${y + 14}" font-size="11" fill="#64748b">${esc(l.cable)} · ${esc(String(l.poles))}P · ${esc(l.phase)}${l.rcd ? " · УЗО " + esc(l.rcd) : ""} · ${esc(String(l.current_a))} А</text>`,
    );
  });
