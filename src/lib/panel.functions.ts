...
${data.lines_text}`;
    ...
    function auditDesign(input: PanelDesign): PanelDesign {
      ...
      // --- реле напряжения: оба варианта должны быть разобраны, выбор обоснован
      ...
      if (Number(data.phases) === 3) {
        ...
        if (!hasA || !hasB) {
          add("error", "Для трёхфазного ввода не разобраны оба варианта контроля напряжения.", "Показать вариант А (три однофазных реле) и вариант Б (общее трёхфазное отключение с контролем фаз и нейтрали): поведение при потере фазы, при аварии N, какие линии отключаются, плюсы и ограничения, и чем обоснован выбор.");
        }
