...
— ГРУППОВЫЕ УЗО, ЭКСПЛУАТАЦИОННАЯ ПРОВЕРКА: помимо токов проверь последствия одного срабатывания. Под одним УЗО 30 мА допускается не более 3 розеточных линий независимых помещений. Если розеточных линий больше — раздели их на две (или более) группы 30 мА тип A, каждой группе назначь свою изолированную N-шину, укажи номинал по сумме токов и опиши причину разделения в note и assumptions. Остальные группы без инженерной необходимости не меняй;
— РЕЛЕ НАПРЯЖЕНИЯ, РЕШЕНИЕ ПРИНИМАЕТ ПРОЕКТ, А НЕ ПОЛЬЗОВАТЕЛЬ: для трёхфазного ввода выполни инженерный выбор сам, по исходным данным (количество фаз, система заземления, наличие и схема N, требование одновременного отключения, расчётный ток ввода и допустимый ток аппарата, необходимость контроля обрыва и смещения N, необходимость контактора и его номинал). Не задавай пользователю вопрос «выберите вариант А или Б». В assumptions коротко сопоставь оба варианта (А — три однофазных реле, Б — общее трёхфазное отключение с контролем всех фаз и нейтрали): поведение при пропадании одной фазы, поведение при аварии нейтрали, что отключается, что остаётся под напряжением, ограничения. Затем прямо напиши «Принято: …» и обоснование. Если принят вариант А — явно укажи, что он не обеспечивает защиту при обрыве нулевого рабочего проводника. Если принят вариант Б — проверь коммутационную способность: при расчётном токе ввода выше номинала реле применяй контактор и укажи его номинал и число модулей. Конкретную модель аппарата не выдумывай: пиши «Для гарантированной защиты требуется аппарат с функцией контроля нулевого рабочего проводника и одновременного отключения всех фаз; конкретная модель должна быть проверена по паспорту производителя»;
— СЕЛЕКТИВНОСТЬ ВВОДНОГО УЗО: никогда не пиши «селективность обеспечена» или «селективность ОК». Формулировка строго такая: «Селективность предварительно обеспечивается по принципу применения временной задержки (тип S), окончательная проверка — по времятоковым характеристикам конкретных аппаратов». Если производитель и модель заданы в исходных данных — сослаться на их характеристики;
— ПАСПОРТНЫЕ ДАННЫЕ: если мощность прибора не задана, запрещено писать «мощность = X кВт». Пиши «Расчётная мощность принята предварительно: X кВт. Паспортная мощность не задана»;
— ВВОДНОЙ КАБЕЛЬ, РАСЧЁТ: если заданы материал жил, способ и условия прокладки, длина и допустимое падение напряжения — выполни расчёт и покажи в assumptions расчётный ток, допустимый длительный ток кабеля, падение напряжения (%), выбранное сечение и запас. Если хотя бы одного данного нет — напиши «Сечение вводного кабеля требует уточнения» и перечисли недостающее в questions.
...
${data.lines_text}`;
    ...
    function auditDesign(input: PanelDesign): PanelDesign {
      ...
      // --- реле напряжения: решение принимает проект, а не пользователь
      const assumptionsText = (d.assumptions ?? []).join(" ").toLowerCase();
      if (Number(data.phases) === 3) {
        const hasA = /три\s+(?:независим|однофазн|отдельн)/.test(assumptionsText);
        const hasB = /(трёхфазн|трехфазн)\w*\s+реле|общ\w+\s+отключен/.test(assumptionsText);
        const decided = /принято\s*[:—-]/.test(assumptionsText);
        if (!hasA || !hasB) {
          add("error", "Для трёхфазного ввода не сопоставлены оба варианта контроля напряжения.", "Показать вариант А (три однофазных реле) и вариант Б (общее трёхфазное отключение с контролем фаз и нейтрали): поведение при потере фазы, при аварии N, какие линии отключаются, ограничения.");
        }
        if (!decided) {
          add("error", "Схема контроля напряжения не выбрана проектом.", "Конструктор обязан сам принять решение по исходным данным и записать «Принято: …» с обоснованием, а не предлагать пользователю выбор.");
        }
        if (/защищ\w*\s+от\s+обрыв\w*\s+(?:н|N)/.test(assumptionsText) && hasA && !hasB) {
          add("error", "Заявлена защита от обрыва нейтрали тремя однофазными реле.", "Три однофазных реле реагируют только на выход напряжения своей фазы за уставку и не дают одновременного отключения. Убрать это утверждение или применить трёхфазное реле с контролем N.");
        }
        // Выбранный вариант должен быть проверен по коммутационной способности.
        const relayItem = (d.rails ?? [])
          .flatMap((r) => r.items ?? [])
          .find((i) => /реле|KV/i.test(`${i.mark} ${i.label}`));
        const relayA = Number(/(\d+)\s*А/.exec(relayItem?.label ?? "")?.[1] ?? 0);
        const calcA = Number(d.summary?.calculated_power_kw ?? 0) * 1000 / (400 * 1.732 * 0.95);
        const singleA = Number(d.summary?.total_power_kw ?? 0) * 1000 / (230 * 3 * 0.95);
        const needA = Math.round(Math.max(calcA, singleA, mainA));
        if (relayA && needA && relayA < needA) {
          add("error", `Коммутационная способность реле напряжения (${relayA} А) ниже расчётного тока ввода ${needA} А.`, "Применить реле большего номинала либо коммутацию через контактор с током не ниже номинала вводного автомата и указать его модули в раскладке.");
        }
        const commonOff = /принято[^.]*(трёхфазн|трехфазн|общ\w+\s+отключен)/.test(assumptionsText);
        if (commonOff && !/контактор/.test(assumptionsText)) {
          add("warning", "Принято общее трёхфазное отключение, но необходимость контактора не проверена.", "Указать: достаточно ли коммутационной способности реле при расчётном токе ввода; если нет — номинал контактора и его модули.");
        }
        if (!/паспорт\w*\s+производител/.test(assumptionsText)) {
          add("warning", "Не указано, что конкретный аппарат контроля напряжения проверяется по паспорту.", "Добавить: «Для гарантированной защиты требуется аппарат с функцией контроля нулевого рабочего проводника и одновременного отключения всех фаз; конкретная модель должна быть проверена по паспорту производителя».");
        }
      }
      // Вопрос о выборе схемы реле пользователю не задаём — решение принимает проект.
      d = {
        ...d,
        questions: (d.questions ?? []).filter(
          (q) => !/реле[^?]*(вариант|выбер|какой)|вариант\s*[аб]\b/i.test(String(q)),
        ),
      };

      // --- вводной кабель: без исходных данных сечение не назначается
      const cableInfo = String(data.input_cable ?? "").trim();
      if (!cableInfo) {
        add("warning", "Вводной кабель не задан — сечение ввода не определено.", "Требуется уточнение: материал жил, способ и условия прокладки, длина трассы, допустимое падение напряжения. Универсального минимального сечения под номинал вводного автомата не существует.");
      } else if (!/\d/.test(cableInfo) || !/(мед|алюм|ввг|nym|авбб|вббш|cu|al)/i.test(cableInfo)) {
        add("warning", `Вводной кабель указан как «${cableInfo}» — данных для проверки сечения недостаточно.`, "Требуется уточнение: марка и материал кабеля, способ прокладки, длина, условия окружающей среды.");
      }
